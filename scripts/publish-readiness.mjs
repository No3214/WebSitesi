import fs from "node:fs";
import path from "node:path";

import { evaluateCommercialLaunch } from "./commercial-launch-audit.mjs";

const root = process.cwd();

const requiredFiles = [
  ".github/workflows/ci.yml",
  ".env.example",
  "docs/evidence/README.md",
  "docs/github-actions-readiness.md",
  "docs/launch-readiness.md",
  "docs/publish-target.md",
  "docs/vercel-operations.md",
  "lighthouserc.json",
  "next.config.ts",
  "scripts/clean-next-build.mjs",
  "scripts/abuse-controls-readiness.mjs",
  "scripts/analytics-readiness.mjs",
  "scripts/brand-integrity-readiness.mjs",
  "scripts/domain-readiness.mjs",
  "scripts/evidence-handoff.mjs",
  "scripts/vercel-env-operator-guidance.mjs",
  "scripts/evidence-template.mjs",
  "scripts/evidence-redaction-scan.mjs",
  "scripts/hero-media-audit.mjs",
  "scripts/media-playback-readiness.mjs",
  "scripts/hms-booking-readiness.mjs",
  "scripts/garanti-pos-readiness.mjs",
  "scripts/admin-surface-readiness.mjs",
  "scripts/webhook-surface-readiness.mjs",
  "scripts/security-headers-readiness.mjs",
  "scripts/supabase-security-readiness.mjs",
  "scripts/github-ci-readiness.mjs",
  "scripts/local-preview-verify.mjs",
  "scripts/live-production-verify.mjs",
  "scripts/production-cutover-plan.mjs",
  "scripts/readiness-summary.mjs",
  "scripts/safe-report-output.mjs",
  "scripts/search-local-seo-readiness.mjs",
  "scripts/vercel-ops-readiness.mjs",
  "scripts/vercel-env-readiness.mjs",
  "scripts/vercel-production-run.mjs",
  "scripts/vercel-commercial-verify.mjs",
  "src/app/api/health/route.ts",
  "src/lib/ecc-auth.ts",
  "src/lib/gtm.ts",
  "src/lib/logger.ts",
  "src/lib/production-readiness.ts",
  "src/lib/security.ts",
  "src/lib/utils.ts",
  "src/lib/webhook-body-limit.ts",
  "src/app/robots.ts",
  "src/app/sitemap.ts",
  "src/app/manifest.ts",
  "tests/agentic-helper-safety.test.ts",
  "tests/abuse-controls-readiness.test.ts",
  "tests/analytics-readiness.test.ts",
  "tests/brand-integrity-readiness.test.ts",
  "tests/ecc-auth.test.ts",
  "tests/garanti-pos-readiness.test.ts",
  "tests/gtm.test.ts",
  "tests/logger.test.ts",
  "tests/admin-surface-readiness.test.ts",
  "tests/security-es256.test.ts",
  "tests/security-utils.test.ts",
  "tests/webhook-body-limit.test.ts",
  "tests/webhook-surface-readiness.test.ts",
  "tests/security-headers-readiness.test.ts",
  "tests/supabase-security-readiness.test.ts",
  "tests/utils.test.ts",
  "tests/local-preview-verify.test.ts",
  "tests/localized-routes.test.ts",
  "tests/search-local-seo-readiness.test.ts",
  "tests/vercel-ops-readiness.test.ts",
  "tests/e2e/health.spec.ts",
  "tests/e2e/publish-routes.spec.ts",
  "tests/production-readiness.test.ts",
  "tests/vercel-production-run.test.ts",
  "tests/vercel-commercial-verify.test.ts",
];

const publicRoutes = [
  "/",
  "/rezervasyon",
  "/odalar",
  "/odalar/standart-bahce-manzarali-oda",
  "/gastronomi",
  "/menu",
  "/organizasyonlar",
  "/misafir-rehberi",
  "/hikayemiz",
  "/deneyimler",
  "/deneyimler/kozbeyli-koyu-rehberi",
  "/deneyimler/foca-gezi-rehberi",
  "/deneyimler/ege-gastronomi-rotasi",
  "/deneyim-tasarimcisi",
  "/teklifler",
  "/galeri",
  "/sss",
  "/iletisim",
  "/lokasyon",
  "/odeme",
  "/kvkk",
  "/gizlilik-politikasi",
  "/cerez-politikasi",
  "/mesafeli-satis-sozlesmesi",
  "/en",
  "/en/booking",
  "/en/rooms",
  "/en/rooms/standart-bahce-manzarali-oda",
  "/en/dining",
  "/en/menu",
  "/en/events",
  "/en/guest-guide",
  "/en/our-story",
  "/en/experiences",
  "/en/offers",
  "/en/gallery",
  "/en/faq",
  "/en/contact",
  "/en/location",
];

const requiredEnvExampleKeys = [
  "NEXT_PUBLIC_SITE_URL",
  "DATABASE_URI",
  "PAYLOAD_SECRET",
  "NEXT_PUBLIC_HMS_BOOKING_ENGINE_URL",
  "NEXT_PUBLIC_WHATSAPP_URL",
  "HOTELRUNNER_WEBHOOK_SECRET",
  "IYZICO_WEBHOOK_SECRET",
  "GARANTI_POS_MODE",
  "GARANTI_MERCHANT_ID",
  "GARANTI_TERMINAL_ID",
  "GARANTI_PROVISION_USER",
  "GARANTI_3D_STORE_KEY",
  "HMS_WEBHOOK_ES256_PUBLIC_KEY",
  "B2B_PARTNER_PUBLIC_KEY",
  "B2B_ALLOW_STATIC_AVAILABILITY",
  "NEXT_PUBLIC_GTM_ID",
  "NEXT_PUBLIC_GA4_MEASUREMENT_ID",
  "NEXT_PUBLIC_GOOGLE_ADS_ID",
  "NEXT_PUBLIC_META_PIXEL_ID",
  "GOOGLE_SITE_VERIFICATION",
  "FACEBOOK_DOMAIN_VERIFICATION",
  "GA4_MEASUREMENT_ID",
  "GA4_API_SECRET",
  "UPSTASH_REDIS_REST_URL",
  "UPSTASH_REDIS_REST_TOKEN",
  "NEXT_PUBLIC_TURNSTILE_SITE_KEY",
  "TURNSTILE_SECRET_KEY",
];

function exists(relPath) {
  return fs.existsSync(path.join(root, relPath));
}

function read(relPath) {
  return fs.readFileSync(path.join(root, relPath), "utf8");
}

function failLine(label, items) {
  if (items.length === 0) return `PASS ${label}: ok`;
  return `FAIL ${label}: ${items.join(", ")}`;
}

const missingFiles = requiredFiles.filter((file) => !exists(file));
const envExample = exists(".env.example") ? read(".env.example") : "";
const missingEnvExampleKeys = requiredEnvExampleKeys.filter(
  (key) => !new RegExp(`^${key}=`, "m").test(envExample),
);

const sitemapSource = exists("src/app/sitemap.ts") ? read("src/app/sitemap.ts") : "";
function sitemapSourceCoversRoute(route) {
  if (route === "/") return sitemapSource.includes("''");
  if (route.startsWith("/en/rooms/")) return true;
  if (route.startsWith("/odalar/")) return true;

  if (route === "/en") {
    return sitemapSource.includes("EN_ROUTE_BY_TR_ROUTE") && sitemapSource.includes("'': '/en'");
  }

  if (route.startsWith("/en/")) {
    return sitemapSource.includes(`'${route}'`) || sitemapSource.includes(`\"${route}\"`);
  }

  return sitemapSource.includes(`'${route}'`) || sitemapSource.includes(`\"${route}\"`);
}

const missingSitemapRoutes = publicRoutes
  .filter((route) => !route.includes("["))
  .filter((route) => !sitemapSourceCoversRoute(route));

const packageJson = JSON.parse(read("package.json"));
const requiredScripts = [
  "lint",
  "typecheck",
  "quality",
  "test:unit",
  "test:e2e",
  "test:monkey",
  "test:chaos",
  "test:stress",
  "security:audit",
  "brand:verify",
  "brand:verify:json",
  "brand:verify:strict",
  "evidence:scan",
  "evidence:scan:json",
  "evidence:handoff",
  "evidence:handoff:json",
  "evidence:handoff:live",
  "evidence:handoff:live:json",
  "evidence:handoff:strict",
  "evidence:templates",
  "evidence:templates:json",
  "evidence:templates:live",
  "evidence:templates:live:json",
  "evidence:templates:live:compact",
  "evidence:templates:live:compact:json",
  "evidence:templates:live:runtime-ready",
  "evidence:templates:live:runtime-ready:json",
  "evidence:templates:live:write",
  "evidence:templates:live:write:json",
  "evidence:templates:live:write:compact",
  "evidence:templates:live:write:compact:json",
  "evidence:templates:live:write:runtime-ready",
  "evidence:templates:live:write:runtime-ready:json",
  "media:hero",
  "media:hero:json",
  "media:hero:strict",
  "media:playback",
  "media:playback:preview",
  "media:playback:live",
  "security:headers",
  "security:headers:json",
  "security:headers:strict",
  "admin:verify",
  "admin:verify:json",
  "admin:verify:strict",
  "webhook:verify",
  "webhook:verify:json",
  "webhook:verify:strict",
  "launch:audit",
  "launch:audit:json",
  "launch:audit:live",
  "launch:audit:live:json",
  "launch:audit:live:strict",
  "launch:audit:strict",
  "abuse:verify",
  "abuse:verify:json",
  "abuse:verify:strict",
  "analytics:verify",
  "analytics:verify:json",
  "analytics:verify:strict",
  "search:verify",
  "search:verify:json",
  "search:verify:strict",
  "garanti:verify",
  "garanti:verify:json",
  "garanti:verify:strict",
  "supabase:verify",
  "supabase:verify:json",
  "supabase:verify:strict",
  "launch:cutover",
  "launch:cutover:json",
  "launch:cutover:strict",
  "launch:standup",
  "launch:standup:json",
  "launch:standup:compact",
  "launch:standup:compact:json",
  "launch:standup:live",
  "launch:standup:live:json",
  "launch:standup:live:compact",
  "launch:standup:live:compact:json",
  "launch:standup:live:write",
  "launch:standup:live:write:json",
  "launch:standup:live:write:compact",
  "launch:standup:live:write:compact:json",
  "launch:standup:strict",
  "hms:verify",
  "hms:verify:json",
  "hms:verify:strict",
  "domain:verify",
  "domain:verify:json",
  "domain:verify:strict",
  "vercel:ops",
  "vercel:ops:json",
  "vercel:ops:strict",
  "vercel:env",
  "vercel:env:json",
  "vercel:env:strict",
  "vercel:env:values",
  "vercel:env:values:strict",
  "vercel:supabase:verify",
  "vercel:abuse:verify",
  "vercel:hms:verify",
  "vercel:garanti:verify",
  "vercel:analytics:verify",
  "vercel:search:verify",
  "vercel:commercial:verify",
  "vercel:commercial:verify:json",
  "github:ci",
  "github:ci:json",
  "github:ci:strict",
  "readiness:summary",
  "readiness:summary:json",
  "readiness:summary:strict",
  "live:verify",
  "live:verify:list",
  "launch:smoke",
  "launch:smoke:preview",
  "launch:smoke:live",
  "localization:verify",
  "localization:verify:preview",
  "localization:verify:live",
  "preview:verify",
  "preview:verify:json",
  "preview:verify:strict",
  "release:verify",
  "release:verify:commercial",
  "prebuild",
  "build",
  "publish:routes",
  "publish:verify",
];
const missingScripts = requiredScripts.filter((script) => !packageJson.scripts?.[script]);

const fatal = [...missingFiles, ...missingEnvExampleKeys, ...missingScripts, ...missingSitemapRoutes];
const commercialLaunch = evaluateCommercialLaunch();
const blockedCommercialGates = commercialLaunch.gateResults.filter((gate) => !gate.ready);
const externalGoLiveBlockers = blockedCommercialGates.map((gate) => gate.label);
const commercialProgressNotes = blockedCommercialGates
  .flatMap((gate) =>
    (gate.progressNotes || [])
      .filter((note) => note.startsWith("live validation lane") || note.startsWith("env/fallback lane"))
      .map((note) => `${gate.id}: ${note}`),
  )
  .slice(0, 6);

const report = [
  "Kozbeyli Konagi publish readiness",
  "Target: repo 95/100, marketing publish 90/100, booking/payment 100/100",
  `Current: repo 95/100, marketing publish 90/100, commercial launch ${commercialLaunch.score}/100`,
  failLine("required files", missingFiles),
  failLine("env example keys", missingEnvExampleKeys),
  failLine("package scripts", missingScripts),
  failLine("sitemap public route inventory", missingSitemapRoutes),
  `INFO public route smoke inventory: ${publicRoutes.length} routes`,
  `INFO commercial launch decision: ${commercialLaunch.decision}`,
  `INFO commercial launch blocked gates: ${
    blockedCommercialGates.length === 0 ? "none" : blockedCommercialGates.map((gate) => gate.id).join(", ")
  }`,
  `INFO commercial launch progress notes: ${commercialProgressNotes.join("; ") || "none"}`,
  `INFO external blockers before full commercial launch: ${externalGoLiveBlockers.join("; ") || "none"}`,
  fatal.length === 0
    ? "RESULT marketing publish gate inventory PASS"
    : "RESULT marketing publish gate inventory FAIL",
].join("\n");

console.log(report);
process.exit(fatal.length === 0 ? 0 : 1);
